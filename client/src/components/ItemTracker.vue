...
const props = withDefaults(defineProps<Props>(), {
    ...
    priceData: () => ({dates: [new Date(), new Date(), new Date(), new Date(), new Date(), new Date(), new Date()], prices: [65, 59, 80, 81, 56, 55, 40]}),
    currency: () => '€',
...
const priceChange = computed(() => {
    const latestPrice = Math.round(props.priceData.prices[props.priceData.prices.length - 1] * 100.0) / 100.0;
    const previousPrice = Math.round(props.priceData.prices[props.priceData.prices.length - 2] * 100.0) / 100.0;
...
const chartData = reactive({
    labels: props.priceData.dates.map(x => x.toLocaleDateString()),
    datasets: [
        {
            label: `Price ${props.currency}`,
